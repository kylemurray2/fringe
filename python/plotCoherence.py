...
def cmdLineParse():
    ...
    parser.add_argument(
        "-i", dest="inputDS", type=str, required=True, help="Input Stack VRT"
    )
    parser.add_argument(
        "-w", dest="wtsDS", type=str, required=True, help="Weights dataset"
    )
    parser.add_argument(
        "-y",
        dest="line",
...
def loadData(inps):
    ...
    for slc in range(nslc):
        indata = (
            ds.GetRasterBand(slc + 1)
            .ReadAsArray(xoff, yoff, 2 * Nx + 1, 2 * Ny + 1)
            .flatten()
        )
